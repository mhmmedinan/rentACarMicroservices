...
public class PaymentManager implements PaymentService {
	...
	private void checkIfRentalTotalPrice(String cardNumber,String cardName,String cvv,double price) {
		...
		double amount = paymentRepository.findByCardNumber(cardNumber).getTotalPrice();
		if (amount < price) {
			throw new BusinessException("insufficient balance");
		}
		posCheckService.pay();
		Payment payment1 = paymentRepository.findByCardNumber(cardNumber);
		payment1.setTotalPrice(amount - price);
